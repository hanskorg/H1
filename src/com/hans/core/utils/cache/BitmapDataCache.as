package com.hans.core.utils.cache
{
	...
	public class BitmapDataCache extends BitmapData implements IDisposeAble
	{
		...
		public function BitmapDataCache(target:DisplayObject,rotation:int = 0,stage:Stage = null)
		{
			...
			super(bounds.width + 2,bounds.height + 2,true,0x00FFFFFF);
			var matrix:Matrix = drawTarget.transform.matrix;
//			matrix.rotate(rotation * ratateChange);
			matrix.tx = -Math.round(bounds.x);//取整，防止小数点便宜引起抖动。
			matrix.ty = -Math.round(bounds.y);//取整，防止小数点便宜引起抖动。
			...
			_offsetX = Math.round(bounds.x);//取整，防止小数点便宜引起抖动。
			_offsetY = Math.round(bounds.y);//取整，防止小数点便宜引起抖动。
			//还原
			if(_isRecord)
			{
				target.x = _xRecord;
